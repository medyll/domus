    ...
    pub fn create(props: ProgressProps) -> Element {
        ...
        match props.variant {
            ProgressVariant::Linear => {
                ...
                container
                    .set_attribute("aria-valuenow", &props.value.get().to_string())
                    .unwrap();
                container.set_attribute("aria-valuemin", "0").unwrap();
                container
                    .set_attribute("aria-valuemax", &props.max.to_string())
                    .unwrap();
                ...
                if let Some(color) = &props.color {
                    bar.set_attribute("style", &format!("background-color: {}", color))
                        .unwrap();
                }

                if !props.indeterminate {
                    let percentage =
                        (props.value.get() as f64 / props.max as f64 * 100.0).min(100.0);
                    bar.set_attribute("style", &format!("width: {}%", percentage))
                        .unwrap();
                }
                ...
                if props.show_label {
                    ...
                    label
                        .set_attribute("class", "domius-progress-label")
                        .unwrap();

                    let label_text = if let Some(format) = &props.label_format {
                        format
                            .replace("{value}", &props.value.get().to_string())
                            .replace("{max}", &props.max.to_string())
                    } else {
                        format!(
                            "{}%",
                            (props.value.get() as f64 / props.max as f64 * 100.0) as u8
                        )
                    };
